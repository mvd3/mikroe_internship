 ...
 */
 public $validate = array(
	 ...
	 'recommended_rating' => array(
		 'notBlank' => array(
			 'rule' => array('notBlank'),
			 'message' => array('Rating can\'t be empty!')
		 )
	 )
 );
...
*/
	...
	public function createExcel($data) {
		...
		$objPHPExcel->getProperties()->setCreator("Nikola Bubic")
												 ...
												 ->setTitle("Materials")
												 ->setSubject("Materials")
												 ->setDescription("List of material in all the warehouses.")
												 ->setKeywords("office PHPExcel php material")
		...
		$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue($col++ . $row, 'material_status')
				->setCellValue($col++ . $row, 'service_production')
				->setCellValue($col++ . $row, 'recommended_rating')
		...
		foreach ($data as $d) {
			...
			$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue($col++ . $row, $d['Material']['material_status'])
					->setCellValue($col++ . $row, $d['Material']['service_production'])
					->setCellValue($col++ . $row, $d['Material']['recommended_rating'])
					->setCellValue($col++ . $row, $d['Material']['created'])
					->setCellValue($col++ . $row, $d['Material']['modified']);
		}

		$objPHPExcel->getActiveSheet()->setTitle('materials');
...
*/
	public function loadFromExcel($filePath) {
		...
		foreach($objPHPExcel->getWorksheetIterator() as $worksheet) {
			...
			while($currentRow<=$highestRow) {
				...
				while ($currentColumn<$highestColumnIndex) {
					if ($cnt-->0) {
						...
						$rawData[$currentRow-2]['Material'][$dataFields[$currentColumn]] = $worksheet->getCellByColumnAndRow($currentColumn, $currentRow)->getValue(); // To Material
					}
					$currentColumn++;
				}
				$cnt = 6;
			...
			foreach ($rawData as $data) {
				...
				if ($item->checkCodeExistence($data['Item']['code'])) {
					$data['Item']['code'] = $item->generateCode($data['Item']['item_type']);
				}
				$data['Material']['material_status'] = array_flip($this->statuses)[$data['Material']['material_status']];
				$data['Material']['recommended_rating'] = array_flip($this->recommendations)[$data['Material']['recommended_rating']];
				//Save
		...
		*/
		...
		foreach($objPHPExcel->getWorksheetIterator() as $worksheet) {
			...
			foreach ($rawData as $data) {
				...
				if ($data['Item']['measurement_unit']==null || $data['Item']['item_type']==null || $data['Material']['material_status']==null) {
					continue;
				}
				$this->create();
				$data['Item']['measurement_unit'] = $unit->getByName($data['Item']['measurement_unit'])['MeasurementUnit']['id'];
				...
				$data['Material']['material_status'] = array_flip($this->statuses)[$data['Material']['material_status']];
				$data['Material']['recommended_rating'] = array_flip($this->recommendations)[$data['Material']['recommended_rating']];
				if ($data['Material']['service_production']>0) {
					$data['Material']['service_production'] = true;
				} else {
					$data['Material']['service_production']	 = false;
				}
				//Save
				if ($this->saveAssociated($data, array('validate' => true, 'deep' => true))) {
					array_push($returnMessage['Success'], "Row $currentRow has been succesfully added.\n");
	...
	public function createPDF() {
		...
		$pdf->SetSubject('List of materials');
		$pdf->SetKeywords('TCPDF, PDF, material');
    ...
    $pdf->SetFont('', 'B', 10);
		//Column width
		$w = array(17, 110, 20, 10, 20);
		// column titles
		$header = array('Code', 'Name', 'Status', 'SP', 'Rating');
		//Header
		...
		$rawData = $this->find('all', array(
				'fields' => array(
					'Item.code', 'Item.name', 'material_status', 'service_production', 'recommended_rating'
				)
			)
		);
		//Style for data
		...
		foreach($rawData as $data) {
			$sp = 'No';
			if ($data['Material']['service_production']) {
				$sp = 'Yes';
			}
			$pdf->Cell($w[0], 6, $data['Item']['code'], 'LR', 0, 'L', $fill);
			$pdf->Cell($w[1], 6, $data['Item']['name'], 'LR', 0, 'L', $fill);
			$pdf->Cell($w[2], 6, $data['Material']['material_status'], 'LR', 0, 'R', $fill);
			$pdf->Cell($w[3], 6, $sp, 'LR', 0, 'R', $fill);
			$pdf->Cell($w[4], 6, $data['Material']['recommended_rating'], 'LR', 0, 'R', $fill);
			$pdf->Ln();
			$fill=!$fill;
		}
