 ...
 */
	...
	*/
	public function prepareTypes($current = null) {
	 ...
	 if ($current!=null) {
		 $cond = array(
			 'AND' => array(
				 array('ItemType.class' => 'inventory'),
				 'OR' => array(
	...
	*/
	...
	public function createExcel($data) {
		...
		$objPHPExcel->getProperties()->setCreator("Nikola Bubic")
												 ...
												 ->setSubject("Inventories")
												 ->setDescription("List of inventories in all the warehouses.")
												 ->setKeywords("office PHPExcel php inventories")
												 ->setCategory("inventories");
		...
		$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue($col++ . $row, 'recommended rating')
		...
		foreach ($data as $d) {
			...
			$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue($col++ . $row, $d['Inventory']['type'])
					->setCellValue($col++ . $row, $d['Inventory']['status'])
					->setCellValue($col++ . $row, $d['Inventory']['recommended rating'])
					->setCellValue($col++ . $row, $d['Inventory']['created'])
					->setCellValue($col++ . $row, $d['Inventory']['modified']);
		}

		$objPHPExcel->getActiveSheet()->setTitle('inventories');
	...
	*/
		public function loadFromExcel($filePath) {
			...
			foreach($objPHPExcel->getWorksheetIterator() as $worksheet) {
				...
				while($currentRow<=$highestRow) {
					...
					while ($currentColumn<$highestColumnIndex) {
						if ($cnt-->0) {
							...
							$rawData[$currentRow-2]['Inventory'][$dataFields[$currentColumn]] = $worksheet->getCellByColumnAndRow($currentColumn, $currentRow)->getValue(); // To Material
						}
						$currentColumn++;
					}
					$cnt = 6;
				...
				foreach ($rawData as $data) {
					...
					if ($data['Item']['measurement_unit']==null || $data['Item']['item_type']==null || $data['Inventory']['status']==null) {
						continue;
					}
					$this->create();
					...
					$data['Inventory']['status'] = array_flip($this->statuses)[$data['Inventory']['status']];
					$data['Inventory']['recommended rating'] = array_flip($this->recommendations)[$data['Inventory']['recommended rating']];
					//Save
		...
		public function createPDF() {
			...
			$pdf->SetTitle('Inventories');
			$pdf->SetSubject('List of inventories');
			...
			$rawData = $this->find('all', array(
					'fields' => array(
						'Item.code', 'Item.name', 'status', 'recommended rating'
					)
				)
			);
			//Style for data
			...
			foreach($rawData as $data) {
				...
				$pdf->Cell($w[2], 6, $data['Inventory']['status'], 'LR', 0, 'R', $fill);
				$pdf->Cell($w[3], 6, $data['Inventory']['recommended rating'], 'LR', 0, 'R', $fill);
				$pdf->Ln();
				$fill=!$fill;
			}
