 ...
 */
	public $validate = array(
		...
		'hts_number' => array(
			'checkIfNeeded' => array(
				'rule' => array('check_status', 'status'),
	 ...
	 public function getCompleteData($id) {
		...
		public function createExcel($data) {
			...
			$objPHPExcel->getProperties()->setCreator("Nikola Bubic")
													 ...
													 ->setTitle("Goods")
													 ->setSubject("Goods")
													 ->setDescription("List of goods in all the warehouses.")
													 ->setKeywords("office PHPExcel php goods")
													 ->setCategory("goods");
			...
			foreach ($data as $d) {
				...
				$objPHPExcel->setActiveSheetIndex(0)
						...
						->setCellValue($col++ . $row, $d['Good']['pid'])
						->setCellValue($col++ . $row, $d['Good']['hts_number'])
						->setCellValue($col++ . $row, $d['Good']['tax_group'])
						->setCellValue($col++ . $row, $d['Good']['eccn'])
						->setCellValue($col++ . $row, $d['Good']['release_date'])
						->setCellValue($col++ . $row, $d['Good']['for_distributors'])
						->setCellValue($col++ . $row, $d['Good']['status'])
						->setCellValue($col++ . $row, $d['Good']['created'])
						->setCellValue($col++ . $row, $d['Good']['modified']);
			}
		...
		*/
			public function loadFromExcel($filePath) {
				...
				foreach($objPHPExcel->getWorksheetIterator() as $worksheet) {
					...
					while($currentRow<=$highestRow) {
						...
						while ($currentColumn<$highestColumnIndex) {
							if ($cnt-->0) {
								...
								// $rawData[$currentRow-2]['Good'][$dataFields[$currentColumn]] = $worksheet->getCellByColumnAndRow($currentColumn, $currentRow)->getValue(); // To Material
								if(PHPExcel_Shared_Date::isDateTime($worksheet->getCellByColumnAndRow($currentColumn, $currentRow))) {
									...
									$rawData[$currentRow-2]['Good'][$dataFields[$currentColumn]] = $worksheet->getCellByColumnAndRow($currentColumn, $currentRow)->getValue();
								}
							}
							$currentColumn++;
						}
					...
					foreach ($rawData as $data) {
						...
						if ($data['Item']['measurement_unit']==null || $data['Item']['item_type']==null || $data['Good']['status']==null) {
							continue;
						}
						$this->create();
						...
						$data['Good']['status'] = array_flip($this->statuses)[$data['Good']['status']];
						if($data['Good']['for_distributors']>0) {
							$data['Good']['for_distributors'] = true;
						} else {
							$data['Good']['for_distributors'] = false;
						}
						//Save
			...
			public function createPDF() {
				...
				$pdf->SetTitle('Goods');
				$pdf->SetSubject('List of goods');
				$pdf->SetKeywords('TCPDF, PDF, good');
				...
				$rawData = $this->find('all', array(
						'fields' => array(
							'Item.code', 'Item.name', 'pid', 'hts_number', 'tax_group', 'eccn', 'release_date', 'for_distributors', 'status'
						)
					)
				);
				//Style for data
				...
				foreach($rawData as $data) {
					$fd = 'No';
					if ($data['Good']['for_distributors']) {
						$fd = 'Yes';
					}
					$pdf->Cell($w[0], 6, $data['Item']['code'], 'LR', 0, 'L', $fill);
					$pdf->Cell($w[1], 6, $data['Item']['name'], 'LR', 0, 'L', $fill);
					$pdf->Cell($w[2], 6, $data['Good']['pid'], 'LR', 0, 'R', $fill);
					$pdf->Cell($w[3], 6, $data['Good']['hts_number'], 'LR', 0, 'R', $fill);
					$pdf->Cell($w[4], 6, $data['Good']['tax_group'], 'LR', 0, 'R', $fill);
					$pdf->Cell($w[5], 6, $data['Good']['eccn'], 'LR', 0, 'R', $fill);
					$pdf->Cell($w[6], 6, CakeTime::format('d-m-Y', $data['Good']['release_date']), 'LR', 0, 'R', $fill);
					$pdf->Cell($w[7], 6, $fd, 'LR', 0, 'R', $fill);
					$pdf->Cell($w[8], 6, $data['Good']['status'], 'LR', 0, 'R', $fill);
					$pdf->Ln();
					$fill=!$fill;
				}
