 ...
 */
	...
	*/
	public function prepareTypes($current = null) {
	 ...
	 if ($current!=null) {
		 $cond = array(
			 'AND' => array(
				 array('ItemType.class' => 'consumable'),
				 'OR' => array(
	...
	*/
	...
	public function createExcel($data) {
		...
		$objPHPExcel->getProperties()->setCreator("Nikola Bubic")
												 ...
												 ->setSubject("Consumables")
												 ->setDescription("List of consumables in all the warehouses.")
												 ->setKeywords("office PHPExcel php consumables")
		...
		$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue($col++ . $row, 'consumable_status')
		...
		foreach ($data as $d) {
			...
			$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue($col++ . $row, $d['Consumable']['consumable_status'])
					->setCellValue($col++ . $row, $d['Consumable']['recommended_rating'])
					->setCellValue($col++ . $row, $d['Consumable']['created'])
					->setCellValue($col++ . $row, $d['Consumable']['modified']);
		}
	...
	*/
		public function loadFromExcel($filePath) {
			...
			foreach($objPHPExcel->getWorksheetIterator() as $worksheet) {
				...
				while($currentRow<=$highestRow) {
					...
					while ($currentColumn<$highestColumnIndex) {
						if ($cnt-->0) {
							...
							$rawData[$currentRow-2]['Consumable'][$dataFields[$currentColumn]] = $worksheet->getCellByColumnAndRow($currentColumn, $currentRow)->getValue(); // To Material
						}
						$currentColumn++;
					}
					$cnt = 6;
				...
				foreach ($rawData as $data) {
					...
					if ($data['Item']['measurement_unit']==null || $data['Item']['item_type']==null || $data['Consumable']['consumable_status']==null) {
						continue;
					}
					$this->create();
					...
					$data['Consumable']['consumable_status'] = array_flip($this->statuses)[$data['Consumable']['consumable_status']];
					$data['Consumable']['recommended_rating'] = array_flip($this->recommendations)[$data['Consumable']['recommended_rating']];
					//Save
		...
		public function createPDF() {
			...
			$pdf->SetTitle('Consumables');
			$pdf->SetSubject('List of consumables');
			$pdf->SetKeywords('TCPDF, PDF, consumable');
	    ...
	    $pdf->SetFont('', 'B', 10);
			...
			$rawData = $this->find('all', array(
					'fields' => array(
						'Item.code', 'Item.name', 'consumable_status', 'recommended_rating'
					)
				)
			);
			//Style for data
			...
			foreach($rawData as $data) {
				...
				$pdf->Cell($w[2], 6, $data['Consumable']['consumable_status'], 'LR', 0, 'R', $fill);
				$pdf->Cell($w[3], 6, $data['Consumable']['recommended_rating'], 'LR', 0, 'R', $fill);
				$pdf->Ln();
				$fill=!$fill;
			}
