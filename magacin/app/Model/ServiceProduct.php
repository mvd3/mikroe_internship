 ...
 */
	public $validate = array(
		...
		'hts_number' => array(
			'checkIfNeeded' => array(
				'rule' => array('check_status', 'service_status'),
		...
		'eccn' => array(
			'checkIfNeeded' => array(
				'rule' => array('check_eccn', 'service_status'),
	...
	*/
		public function prepareTypes($current = null) {
		 ...
		 if ($current!=null) {
			 $cond = array(
				 'AND' => array(
					 array('ItemType.class' => 'service_product'),
					 'OR' => array(
	...
	*/
		...
		public function createExcel($data) {
			...
			$objPHPExcel->getProperties()->setCreator("Nikola Bubic")
													 ...
													 ->setSubject("Service Products")
													 ->setDescription("List of service products in all the warehouses.")
													 ->setKeywords("office PHPExcel php serviceproducts")
													 ->setCategory("serviceproducts");
			...
			$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue($col++ . $row, 'release_date')
			...
			foreach ($data as $d) {
				...
				$objPHPExcel->setActiveSheetIndex(0)
						...
						->setCellValue($col++ . $row, $d['ServiceProduct']['pid'])
						->setCellValue($col++ . $row, $d['ServiceProduct']['hts_number'])
						->setCellValue($col++ . $row, $d['ServiceProduct']['tax_group'])
						->setCellValue($col++ . $row, $d['ServiceProduct']['eccn'])
						->setCellValue($col++ . $row, $d['ServiceProduct']['release_date'])
						->setCellValue($col++ . $row, $d['ServiceProduct']['for_distributors'])
						->setCellValue($col++ . $row, $d['ServiceProduct']['service_status'])
						->setCellValue($col++ . $row, $d['ServiceProduct']['project'])
						->setCellValue($col++ . $row, $d['ServiceProduct']['created'])
						->setCellValue($col++ . $row, $d['ServiceProduct']['modified']);
			}

			$objPHPExcel->getActiveSheet()->setTitle('serviceproducts');
		...
		*/
			public function loadFromExcel($filePath) {
				...
				foreach($objPHPExcel->getWorksheetIterator() as $worksheet) {
					...
					while($currentRow<=$highestRow) {
						...
						while ($currentColumn<$highestColumnIndex) {
							if ($cnt-->0) {
								...
								// $rawData[$currentRow-2]['ServiceProduct'][$dataFields[$currentColumn]] = $worksheet->getCellByColumnAndRow($currentColumn, $currentRow)->getValue(); // To Material
								if(PHPExcel_Shared_Date::isDateTime($worksheet->getCellByColumnAndRow($currentColumn, $currentRow))) {
									...
									$rawData[$currentRow-2]['ServiceProduct'][$dataFields[$currentColumn]] = $worksheet->getCellByColumnAndRow($currentColumn, $currentRow)->getValue();
								}
							}
							$currentColumn++;
						}
					...
					foreach ($rawData as $data) {
						...
						if ($data['Item']['measurement_unit']==null || $data['Item']['item_type']==null) {
							continue;
						}
						$this->create();
						...
						$data['ServiceProduct']['service_status'] = array_flip($this->statuses)[$data['ServiceProduct']['service_status']];
						if($data['ServiceProduct']['for_distributors']>0) {
							$data['ServiceProduct']['for_distributors'] = true;
						} else {
							$data['ServiceProduct']['for_distributors'] = false;
						}
						//Save
			...
			public function createPDF() {
				...
				$pdf->SetTitle('Service Products');
				$pdf->SetSubject('List of service products');
				$pdf->SetKeywords('TCPDF, PDF, service, product');
				...
				$pdf->SetFont('', 'B', 10);
				//Column width
				$w = array(17, 120, 15, 15, 15, 15, 20, 10, 20, 15);
				// column titles
				$header = array('Code', 'Name', 'PID', 'HTS', 'Tax', 'ECCN', 'Release', 'Dist', 'Status', 'Project');
				//Header
				...
				$rawData = $this->find('all', array(
						'fields' => array(
							'Item.code', 'Item.name', 'pid', 'hts_number', 'tax_group', 'eccn', 'release_date', 'for_distributors', 'service_status', 'project'
						)
					)
				);
				//Style for data
				...
				foreach($rawData as $data) {
					$fd = 'No';
					if ($data['ServiceProduct']['for_distributors']) {
						$fd = 'Yes';
					}
					$pdf->Cell($w[0], 6, $data['Item']['code'], 'LR', 0, 'L', $fill);
					$pdf->Cell($w[1], 6, $data['Item']['name'], 'LR', 0, 'L', $fill);
					$pdf->Cell($w[2], 6, $data['ServiceProduct']['pid'], 'LR', 0, 'R', $fill);
					$pdf->Cell($w[3], 6, $data['ServiceProduct']['hts_number'], 'LR', 0, 'R', $fill);
					$pdf->Cell($w[4], 6, $data['ServiceProduct']['tax_group'], 'LR', 0, 'R', $fill);
					$pdf->Cell($w[5], 6, $data['ServiceProduct']['eccn'], 'LR', 0, 'R', $fill);
					$pdf->Cell($w[6], 6, CakeTime::format('d-m-Y', $data['ServiceProduct']['release_date']), 'LR', 0, 'R', $fill);
					$pdf->Cell($w[7], 6, $fd, 'LR', 0, 'R', $fill);
					$pdf->Cell($w[8], 6, $data['ServiceProduct']['service_status'], 'LR', 0, 'R', $fill);
					$pdf->Cell($w[9], 6, $data['ServiceProduct']['project'], 'LR', 0, 'R', $fill);
					$pdf->Ln();
					$fill=!$fill;
				}
